...
describe("Revalidate cache test access via bearer token", () => {
  testSilently(
    "That the cache revalidation endpoint returns 401 if no bearer token is provided",
    async () => {
...
describe("Revalidate cache test combination of payloads", () => {
  testSilently("That the cache revalidation endpoint returns 422 upon wrong input", async () => {
    await testApiHandler({
      appHandler: revalidateCacheHandler,
      url: `/cache/revalidate`,
      async test({ fetch }) {
...
describe("Revalidate cache test different path and tag formats", async () => {
  testSilently(
    "That the cache revalidation endpoint returns 422 if a wrongly formatted path is given",
    async () => {
      await testApiHandler({
        appHandler: revalidateCacheHandler,
        url: `/cache/revalidate`,
  ...
  testSilently(
    "That the cache revalidation endpoint returns 422 if a wrongly formatted tag is given",
    async () => {
      await testApiHandler({
        appHandler: revalidateCacheHandler,
        url: `/cache/revalidate`,
